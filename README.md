...
Matrix DMs are encrypted by default in most clients. zot's E2EE support is opt-in and uses the **pure-Go** olm implementation (`goolm`) so the build stays CGO-free. Because the upstream crypto store is gated behind a build tag, E2EE requires building zot with that tag:

```bash
CGO_ENABLED=0 go build -tags goolm ./...   # E2EE-capable binary
```

Without `-tags goolm`, the bot compiles and works for **unencrypted** rooms, but if you configured a `crypto_passphrase` the bot errors out with a clear message rather than silently downgrading. The E2EE key store lives in `$ZOT_HOME/matrix-crypto/store.db` (pickled with your passphrase); `zot matrix-bot reset` wipes it.
